# Fixed installed validation — 2026-09-09 (candidate ded02ac, PR #104 fix for PR #101 stall)
...
- Corrected candidate SHA (Agent 1 fix, PR #104):
  ...
  (PR #99, 10/10 green). Verified by `git -C <worktree> rev-parse HEAD`.
- Exclusive test lock (PR #103):
  ...
  host lock`). The lock helper `scripts/foundation-smoke-lock.ps1` from this
  branch was used for the exclusive window procedure only; it is test
  orchestration and is not compiled into the binary. The binary was built
  from the `ded02ac` worktree, so the product code under test is exactly the
  Agent 1 SHA.
...
- Locks: `Cargo.lock` SHA-256
  `CCCB9275FA4951BE4B03B6C485DF998C7C59D523A8A3735B477D36599284BC95`
  (matches PR #104 body); `pnpm-lock.yaml` SHA-256
...
Merged commit plus remote CI (§1), deterministic suites (PR #104:
`fruitboard-desktop --features scan-console` 92 passed,
`fruitboard-storage` 76 passed, `fruitboard-scan-execution` 56 passed,
Clippy clean — quoted from PR #104, not re-run here except via the
installed build), installed observations (§5), and owner acceptance remain
...
**Recommend merging PR #104 (`ded02ac`) after the standard `pnpm check`
gate, with S5 recorded as PARTIAL as above. Do not declare Phase 2
accepted on this record.**
