...
## Live review status 2026-09-12

This is the sole dated table for frequently changing `main` and PR heads. The
head OIDs were resolved from the fetched Git refs and cross-checked against
live GitHub state before publication. Other current-status prose links here;
historical evidence references elsewhere are intentionally preserved.
...
Agent 1 coordination is represented by the live dependency chain in this
table: #111 is based on #110, #112 is based on #111, and the final source-stack
head is therefore #112. #113 remains a separate older combined candidate.
...
## Unmerged evidence and tested-source boundaries

The current PR heads are kept in the [dated live review status table](#live-review-status-2026-09-12).
...
same-job restart observations are published in PR #110, but remain unmerged
and owner-unaccepted. The separate NTFS report in PR #111 was published at
reviewed revision
`ef085229471301043a50f4b668901d9c956fb407`; it was tested from unmerged source
...
terminal queued/running states. Those artifacts are isolated in PR #110;
Agent 2 found no defect and no contract change is proposed. Draft #111's
...
The current source dependencies are #110 → #111 → #112: #111 depends on
the #110 evidence carrier, and #112 depends on #111's mixed product/evidence
tip. PR #112 is the current source-stack head in the dated status table. The
...
source-stack order is #110 → #111 → #112, as recorded in the dated status
table. PR #113 is an older synthetic combined candidate based on `main`, not
